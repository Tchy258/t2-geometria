...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cout << "Usage ./IntegerAtLeastThreeColinearTest.exe <times per size> [seed] [<filename>]" << std::endl;
        return 1;
    }
    int seed = 123;
    if (argc > 2) {
    ...
    auto strategy = std::unique_ptr<PointGenerationStrategy<int>>(new AtLeastThreeColinearPointsStrategy<int>());
