...
int main(int argc, char** argv) {
    if (argc < 3) {
        std::cout << "Usage ./LLHullTest.exe <times per size> <percentage on hull from 0 to 100> [<colinear point amount?] [seed] [<filename>]" << std::endl;
        return 1;
    }
    ...
    executeTest<long long>(atoi(argv[1]), filename, -159200LL, 159200LL, seed, std::move(strategy), std::move(hullStrategy), std::move(hullStrategy2));
}
